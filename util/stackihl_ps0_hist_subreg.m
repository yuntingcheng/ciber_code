...
%%% determine the subregion of sources %%%
...
stackcount = zeros([1,8]);
for ireg=1:8
    stackcount(ireg)=numel(find(regidx==ireg));
end
...
nbins = 25;
if Nsub == 0
    histcb = zeros(8,nbins,numel(Ibinedges_cb)-1);
    histps = zeros(8,nbins,numel(Ibinedges_ps)-1);
else
    histcb = zeros(8,Nsub,nbins,numel(Ibinedges_cb)-1);
    histps = zeros(8,Nsub,nbins,numel(Ibinedges_ps)-1);
end    
...
for i=idx_stack_arr
    ...
    for ibin = 1:nbins
        ...
        if Nsub == 0
            histcb(ri,ibin,:) = histcb(ri,ibin,:) ...
                + reshape(N,[1,numel(Ibinedges_cb)-1]);
        else
            histcb(ri,rem(i,Nsub)+1,ibin,:) = histcb(ri,rem(i,Nsub)+1,ibin,:) + ...
                reshape(N,[1,1,1,numel(Ibinedges_cb)-1]);
        end
        ...
        if Nsub == 0
            histps(ri,ibin,:) = histps(ri,ibin,:) ...
                + reshape(N,[1,numel(Ibinedges_ps)-1]);
        else
            histps(ri,rem(i,Nsub)+1,ibin,:) = histps(ri,rem(i,Nsub)+1,ibin,:) + ...
                reshape(N,[1,1,1,numel(Ibinedges_ps)-1]);
        end
