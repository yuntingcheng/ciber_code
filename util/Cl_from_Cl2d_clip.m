%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Cl_from_Cl2d_clip: almost same as Cl_from_Cl2d.m except mask out crazy 
% pixels in weighted 2DPS
% Inputs (Required):
%
...
function [Cl,Cl2dw,fmask,l, lth, dCl, binl, dl, Cl2d, lind, nell] = ...
    Cl_from_Cl2d_clip(Cl2d,pixscale,varargin)
  ...
  p.addOptional('sigclip',zeros(1,29),@isnumeric);
  p.addOptional('smsig',0,@isnumeric);
  p.addOptional('hsize_r',1,@isnumeric);
  ...
  smsig  = p.Results.smsig;
  hsize_r  = p.Results.hsize_r;
  sigclip  = p.Results.sigclip;
  ...
  if smsig
      fftsq=fillpadsmooth(log10(fftsq),ones(1024),smsig,'hsize_r',1);
      fftsq=10.^fftsq;
  end
  ...
  if ~superbin
    ...
    lind = NaN;
      ...
      for i = 1:nl-1
          stuffw = fftsqw((l >= binl(i)) & (l <= binl(i+1)));
          stuff=fftsq((l >= binl(i)) & (l <= binl(i+1)));
          lower = w((l >= binl(i)) & (l <= binl(i+1)));
          ind = find((l >= binl(i)) & (l <= binl(i+1)));
          mask= fmask((l >= binl(i)) & (l <= binl(i+1)));

          if ~isempty(stuffw) & isfinite(sum(stuffw(lower ~= 0)))
              %%%% do the sigma clipping here%%%%
              clipmax=median(stuff(lower~=0))+std(stuff(lower~=0))*sigclip(i);
              if sigclip(i)~=0
                lower(stuff>clipmax)=0;
                mask(stuff>clipmax)=0;
                fmask(ind)=mask;
              end
              Cl(i) = sum(stuffw(lower ~= 0))/...
                      sum(lower(lower ~= 0));
              nell(i) = numel(lower(lower ~= 0));
          else
              Cl(i) = 0;
          end
  ...
  %% correct dCl in the last bin
  A_frac=(2*binl(end)^2-pi*binl(end-1)^2)/(pi*(binl(end)^2-binl(end-1)^2));
  dCl(end)=dCl(end)/sqrt(A_frac);
  %% Arcmin.
  lth = 2*60*180.0./l;
  dl = bin;
