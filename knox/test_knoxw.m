...
savedir='/Users/ytcheng/ciber/doc/20160808_DarkProcess/test_knox/womask/';
...
logstdsm=fillpadsmooth(log10(darkps(8).Clf2d_std),ones(1024),5,'hsize_r',1);
fmask=zeros(1024);
...
savename=strcat(sprintf('%sfmask/ell%d',savedir,i));
print(savename,'-dpng');%close

fmask(find(cmask))=1;

end
%% plot Fourier mask (whole map)
setwinsize(gcf,1500,300)

a=log10(darkps(8).Clf2d_std).*fmask;m=median(a(find(a)));
subplot(1,3,1)
imageclip(log10(darkps(8).Clf2d_std)-m);
v=caxis;

subplot(1,3,2)
imageclip((log10(darkps(8).Clf2d_std)-m).*fmask);
xlim([min(x),max(x)]); ylim([min(y),max(y)]);
caxis(v)
...
savename=strcat(sprintf('%sfmask/elltot',savedir));
print(savename,'-dpng');%close
%%
weight=(1./darkps(8).Clf2d_std)';
mweight=(1./darkps(8).Clf2d_std)'.*fmask';
%% 100 white noise sim
...
savename=strcat(savedir,'knox_sim_weight0');
print(savename,'-dpng');%close
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
weight0=(1./(darkps(8).Clf2d_std))';
mweight0=(1./(darkps(8).Clf2d_std))'.*fmask';

weight1=(1./(darkps(8).Clf2d_ave))';
mweight1=(1./(darkps(8).Clf2d_ave))'.*fmask';

weight2=(1./(darkps(8).Clf2d_std.^2))';
mweight2=(1./(darkps(8).Clf2d_std.^2))'.*fmask';

weight3=(1./(darkps(8).Clf2d_ave+darkps(8).Clf2d_std))';
mweight3=(1./(darkps(8).Clf2d_ave+darkps(8).Clf2d_std))'.*fmask';
...
for i=1:100
    ...
    [Clw] = Cl_from_Cl2d_clip(Cl2d,pixscale,'w',weight0);
    [Clmw] = Cl_from_Cl2d_clip(Cl2d,pixscale,'w',mweight0);
    Clsw_arr(i,:)=Clw;
    Clsmw_arr(i,:)=Clmw;
...
ylabel('$\delta C_\ell/\bar{C_\ell}$',...
        'interpreter','latex','fontsize',18)
xlim([1e2,2e5]);ylim([1e-3,2e0]);
savename=strcat(savedir,'knox_sim_rand_weight0');
print(savename,'-dpng');%close
%%%%%%%
figure
loglog(l,dCl,'k');hold on
...
ylabel('$\delta C_\ell/\bar{C_\ell}$',...
        'interpreter','latex','fontsize',18)
xlim([1e2,2e5]);ylim([1e-3,2e0]);
savename=strcat(savedir,'knox_sim_rand_weight1');
print(savename,'-dpng');%close
%%%%%%%
figure
loglog(l,dCl,'k');hold on
...
ylabel('$\delta C_\ell/\bar{C_\ell}$',...
        'interpreter','latex','fontsize',18)
xlim([1e2,2e5]);ylim([1e-3,2e0]);
savename=strcat(savedir,'knox_sim_rand_weight2');
print(savename,'-dpng');%close
...
ylabel('$\delta C_\ell/\bar{C_\ell}$',...
        'interpreter','latex','fontsize',18)
xlim([1e2,2e5]);ylim([1e-3,2e0]);
savename=strcat(savedir,'knox_sim_rand_weight3');
print(savename,'-dpng');%close
%%%%%%%%
%%
figure
pltnw=loglog(l,l.*(l+1).*mean(Cln_arr).*cal.^2./2./pi,'k.');hold on
errorbar(l,l.*(l+1).*mean(Cln_arr).*cal.^2./2./pi,...
l.*(l+1).*(mean(Cln_arr).*cal.^2-prctile(Cln_arr.*cal.^2,16))./2./pi,...
l.*(l+1).*(prctile(Cln_arr.*cal.^2,84)-mean(Cln_arr).*cal.^2)./2./pi,'k.');

pltw0=loglog(l,l.*(l+1).*mean(Clnw_arr).*cal.^2./2./pi,'b.');hold on
errorbar(l,l.*(l+1).*mean(Clnw_arr).*cal.^2./2./pi,...
l.*(l+1).*(mean(Clnw_arr).*cal.^2-prctile(Clnw_arr.*cal.^2,16))./2./pi,...
l.*(l+1).*(prctile(Clnw_arr.*cal.^2,84)-mean(Clnw_arr).*cal.^2)./2./pi,'b.');

pltw1=loglog(l,l.*(l+1).*mean(Clnw1_arr).*cal.^2./2./pi,'g.');hold on
errorbar(l,l.*(l+1).*mean(Clnw1_arr).*cal.^2./2./pi,...
l.*(l+1).*(mean(Clnw1_arr).*cal.^2-prctile(Clnw1_arr.*cal.^2,16))./2./pi,...
l.*(l+1).*(prctile(Clnw1_arr.*cal.^2,84)-mean(Clnw1_arr).*cal.^2)./2./pi,'g.');

pltw2=loglog(l,l.*(l+1).*mean(Clnw2_arr).*cal.^2./2./pi,'r.');hold on
errorbar(l,l.*(l+1).*mean(Clnw2_arr).*cal.^2./2./pi,...
l.*(l+1).*(mean(Clnw2_arr).*cal.^2-prctile(Clnw2_arr.*cal.^2,16))./2./pi,...
l.*(l+1).*(prctile(Clnw2_arr.*cal.^2,84)-mean(Clnw2_arr).*cal.^2)./2./pi,'r.');

pltw3=loglog(l,l.*(l+1).*mean(Clnw3_arr).*cal.^2./2./pi,'m.');hold on
errorbar(l,l.*(l+1).*mean(Clnw3_arr).*cal.^2./2./pi,...
l.*(l+1).*(mean(Clnw3_arr).*cal.^2-prctile(Clnw3_arr.*cal.^2,16))./2./pi,...
l.*(l+1).*(prctile(Clnw3_arr.*cal.^2,84)-mean(Clnw3_arr).*cal.^2)./2./pi,'m.');

xlabel('$\ell$','interpreter','latex','fontsize',18)
ylabel('$\ell(\ell+1)C_\ell/2\pi(nW m^{-2} sr^{-1})$',...
        'interpreter','latex','fontsize',18)
legend([pltnw,pltw0,pltw1,pltw2,pltw3],...
{'noweight','weight0','weight1','weight2','weight3'},...
...
figure
loglog(l,mean(Cln_arr),'ko-');hold on
loglog(l,mean(Clnw_arr),'bo-')
loglog(l,mean(Clnw1_arr),'go-')
loglog(l,mean(Clnw2_arr),'ro-')
loglog(l,mean(Clnw3_arr),'mo-')
xlim([1e2,2e5]);
xlabel('$\ell$','interpreter','latex','fontsize',18)
...
figure
loglog(l,std(Cln_arr),'ko-');hold on
loglog(l,std(Clnw_arr),'bo-')
loglog(l,std(Clnw1_arr),'go-')
loglog(l,std(Clnw2_arr),'ro-')
loglog(l,std(Clnw3_arr),'mo-')
xlim([1e2,2e5]);
xlabel('$\ell$','interpreter','latex','fontsize',18)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% real RN data 
Cl2d_arr=darkps(8).Clf2d_arr;
Cl_arr=darkps(8).Clf_arr;
