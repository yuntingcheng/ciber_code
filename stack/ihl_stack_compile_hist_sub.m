function ihl_stack_compile_hist_sub(flight,inst,ifield,reg)
%%
...
elseif strcmp(reg,'E')
    reg_arr = [1,3,5,7];
elseif strcmp(reg,'Q1')
    reg_arr = [1,2];
elseif strcmp(reg,'Q2')
    reg_arr = [3,4];
elseif strcmp(reg,'Q3')
    reg_arr = [5,6];
elseif strcmp(reg,'Q4')
    reg_arr = [7,8];
else
    error('reg name not valid.')
end
...
for im=10:12
    ...
    histpsg = 0;
    for isub=reg_arr
        load(sprintf('%shistdat_%s_subreg%d',loaddir,dt.name,isub),'histdat');
        counts_arr = [counts_arr,histdat(im).counts];
        countg_arr = [countg_arr,histdat(im).countg];
        histcbs = histcbs + histdat(im).histcbs;
        histcbg = histcbg + histdat(im).histcbg;
        histpss = histpss + histdat(im).histpss;
        histpsg = histpsg + histdat(im).histpsg;
    end
    histdattot(im).Ibinedges_cb = histdat(im).Ibinedges_cb;
    histdattot(im).Ibinedges_ps = histdat(im).Ibinedges_ps;
    histdattot(im).counts_arr = counts_arr;
    histdattot(im).countg_arr = countg_arr;
    histdattot(im).counts = sum(counts_arr);
    histdattot(im).countg = sum(countg_arr);
    histdattot(im).histcbs = histcbs;
    histdattot(im).histcbg = histcbg;
    histdattot(im).histpss = histpss;
    histdattot(im).histpsg = histpsg;
end
histdat = histdattot;
clear histdattot
%%
...
for im=10:12
    ...
    counts_arr = histdat(im).counts_arr;
    countg_arr = histdat(im).countg_arr;
    ...
    for isub=reg_arr
        c=c+1;
        load(strcat(bkdir,'bk_ps/',dt.name,'_histbkdat_subreg',...
            num2str(isub),'_1'),'histbkdat');

        for i=1:size(histbkdat,2)
            if histbkdat(i).N == counts_arr(c)
                bkidx_s_arr = [bkidx_s_arr,i];
            end
            if histbkdat(i).N == countg_arr(c)
                bkidx_g_arr = [bkidx_g_arr,i];
            end  
        end
    end
        ...
        for itype=1:4
            ...
            for i=1:nsim
                ...
                for isub=reg_arr
                    c=c+1;
                    load(strcat(bkdir,'bk_ps/',dt.name,'_histbkdat_subreg',...
                        num2str(isub),'_',num2str(i)),'histbkdat');
                    if itype==1
                        d = d + histbkdat(bkidx_s_arr(c)).histcb(ibin,:);
                    elseif itype==2
                        d = d + histbkdat(bkidx_g_arr(c)).histcb(ibin,:);
                    elseif itype==3
                        d = d + histbkdat(bkidx_s_arr(c)).histps(ibin,:);
                    elseif itype==4
                        d = d + histbkdat(bkidx_g_arr(c)).histps(ibin,:);
                    end
                end
                d = reshape(d, size(bins));
                spi = find((d~=0) & (bins < clipmax) & (bins > clipmin));
...
loaddir=strcat(mypaths.alldat,'TM',num2str(inst));
save(sprintf('%s/%s_ihlprofdat_hist_%s',loaddir,dt.name,reg),'ihlprofdat');
