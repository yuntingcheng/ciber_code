%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Comparing the total source flux of CIBER, UKIDSS sim map, UK catalog flux. 
...
%% plot 1x1 3x3 5x5 7x7

for quad=quad_arr
figure
setwinsize(gcf,1000,1000)
for inpix=1:numel(npix_arr)
subplot(2,2,inpix)
npix=npix_arr(inpix);
...
errorbar(x_arr,y_arr,ey_arr,ey_arr,ex_arr,ex_arr,'ro',...
    'DisplayName','optimal');hold on
%%%%%%%%%% sum %%%%%%%%%%%%%
x_arr=zeros(1,numel(Iedge_arr)-1);
ex_arr=zeros(1,numel(Iedge_arr)-1);
y_arr=zeros(1,numel(Iedge_arr)-1);
ey_arr=zeros(1,numel(Iedge_arr)-1);
for iI=1:numel(Iedge_arr)-1
    sp=find(Ix1_arr>Iedge_arr(iI) & Ix1_arr<=Iedge_arr(iI+1));
    yuse=Iy1_arr(sp);
    x_arr(iI)=nanmedian(Ix1_arr(sp));
    ex_arr(iI)=std(Ix1_arr(sp))./sqrt(numel(sp));
    y_arr(iI)=nanmedian(yuse);
    ey_arr(iI)=std(yuse)./sqrt(numel(sp));
...
errorbar(x_arr,y_arr,ey_arr,ey_arr,ex_arr,ex_arr,'b.',...
    'DisplayName','sum');hold on
set(gca,'xscale','log')
set(gca,'yscale','log')
xlim([1e-6,1e0])
ylim([1e-6,1e0])
plot([1e-6,1e0],[1e-6,1e0],'k','DisplayName','x=y');
...
imname=strcat(savedir,dt.name,'_stampsize_',sname,'_',quad);
%print(imname,'-dpng');%close
end
%% plot 3x3 different data
...
imname=strcat(savedir,dt.name,'_data_',sname,'_',quad);
print(imname,'-dpng');%close
end
...
if inst==1
    r_arr=0.5:0.001:0.7;%inst1
else
    r_arr=0.2:0.001:0.6;%inst2
end
...
Ix_arr=Icat_arr;Ix1_arr=Icat_arr;xlab=ukcatlab;
Iy_arr=Icb_arr;Iy1_arr=Icb1_arr;ylab=cbdatlab;
sname=strcat(dt.name,' UKIDSS input');
...
for ir=1:numel(r_arr)
    ...
    %chi2in=chi2in(4:end-3);
    chi2_arr(ir)=nansum(chi2in);
end
dof=numel(find(chi2in==chi2in))-1;
bestr=r_arr(find(chi2_arr==min(chi2_arr)));
plot(r_arr,chi2_arr./dof,'DisplayName',...
    sprintf('%s, F_{CIBER}/F_{UKIDSS}=%.3f',sname,bestr));hold on
end

ylim([0,5])
xlabel('$F_{CIBER}/F_{UKIDSS}$','interpreter','latex','fontsize',18)
ylabel('$\chi^2/dof$','interpreter','latex','fontsize',18)
h=legend('show','Location','northwest');
set(h,'fontsize',10)
legend boxoff
imname=strcat(savedir,'chi2');
print(imname,'-dpng');%close
