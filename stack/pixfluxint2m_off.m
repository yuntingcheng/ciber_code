...
quad_arr=['A','B','C','D'];
npix_arr=[1,3,5,7];
loaddir=strcat('/Users/ytcheng/ciber/doc/20170617_Stacking/simflux/offset/TM',...
    num2str(inst),'/');
savedir=strcat('/Users/ytcheng/ciber/doc/20170617_Stacking/simflux/',...
    'plots/TM',num2str(inst),'/');
...
for quad=quad_arr
fname=strcat(loaddir,dt.name,'_',quad,'_pix',num2str(npix),'_flux_opt_2m_off1.txt');
M = csvread(fname);
...
figtitle(strcat(dt.name,'\_',num2str(npix),'x',num2str(npix)),'fontweight','bold');
imname=strcat(savedir,dt.name,'_data_',sname,'_off1');
if sum==1
    imname=strcat(savedir,dt.name,'_data_',sname,'_off1_sum');
end
print(imname,'-dpng');%close
end
%% fit the scaling factor (do 5 fields in the same plots)
npix=3;
if inst==1
    r_arr=0.5:0.001:0.8;
else
...
set(h,'fontsize',10)
legend boxoff
imname=strcat(savedir,'chi2_2m_off1');
if sum==1
    imname=strcat(savedir,'chi2_2m_off1_sum');
end
print(imname,'-dpng');%close
