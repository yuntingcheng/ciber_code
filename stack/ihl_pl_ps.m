...
quad_arr=['A','B','C','D'];
m_arr=13:17;
load(strcat(savedir,'plprofdat'),'plprofdat');
%%
...
for im=1:numel(m_arr)
    ...
    m=m_arr(im);
    A=plprofdat(5).params(1);
    alpha=plprofdat(5).params(2);
     
    ihlmap=make_ihl_pl(flight,inst,ifield,m,m+1,A,alpha);
...
savename=strcat(savedir,dt.name,'_2m');
...
axes; 
set(gca,'Visible','off'); 
h = title(sprintf('power-law IHL'),'fontsize',20);
set(h,'Visible','on');

savename=strcat(savedir,dt.name,'_map');
print(savename,'-dpng');%close
