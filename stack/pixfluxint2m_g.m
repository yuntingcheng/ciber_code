...
for quad=quad_arr
fname=strcat(loaddir,dt.name,'_',quad,'_pix',num2str(npix),'_flux_opt_g.txt');
M = csvread(fname);
...
imname=strcat(savedir,dt.name,'_data_',sname,'_g');
if sum==1
    imname=strcat(savedir,dt.name,'_data_',sname,'_g_sum');
end
...
set(h,'fontsize',10)
legend boxoff
imname=strcat(savedir,'chi2_2m_g');
if sum==1
    imname=strcat(savedir,'chi2_2m_g_sum');
end
