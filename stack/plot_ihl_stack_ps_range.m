...
ncountfile=strcat('/Users/ytcheng/ciber/doc/20171018_stackihl/stackmaps/TM',...
    num2str(inst),'/panstarrs_ps/',dt.name,'_stackcounts.txt');
...
countg=0;
for m=mmin:mmax-1
    counts=counts+sum(counts_arr(find(mbot_arr==m)));
    countg=countg+sum(countg_arr(find(mbot_arr==m)));
end
...
for iquad=1:4
    ...
    if itype==1
        stacks=fitsread(strcat(loaddir,dt.name,'_',quad,...
            '_stampers',num2str(mmin),'_',num2str(mmax),'.fits'));
        stackg=fitsread(strcat(loaddir,dt.name,'_',quad,...
            '_stamperg',num2str(mmin),'_',num2str(mmax),'.fits'));
    else
        stacks=fitsread(strcat(loaddir,dt.name,'_',quad,...
            '_stampers',num2str(mmin),'_',num2str(mmax),'.fits'));
        stackg=fitsread(strcat(loaddir,dt.name,'_',quad,...
    ...
    mstacks=fitsread(strcat(loaddir,dt.name,'_',quad,...
        '_maskstampers',num2str(mmin),'_',num2str(mmax),'.fits'));
    mstackg=fitsread(strcat(loaddir,dt.name,'_',quad,...
        '_maskstamperg',num2str(mmin),'_',num2str(mmax),'.fits'));
...
suptitle(strcat('PanSTARRS',{' '},num2str(mmin),'<mAB(y band)<',num2str(mmax)));
savename=strcat(savedir,dt.name,'_stackmaps_allquad',...
num2str(mmin),'_',num2str(mmax));
print(savename,'-dpng');
...
savename=strcat(savedir,dt.name,'_rprof_allquad',num2str(mmin),'_',num2str(mmax));
print(savename,'-dpng');
