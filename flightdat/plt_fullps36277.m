...
psdatdir='/Users/ytcheng/ciber/doc/20161102_36277/FlightDat/';
...
phdir=strcat('/Users/ytcheng/ciber/doc/20161102_36277/FlightDat/band',...
    num2str(band),'_ph/');
...
%%% before RN sub
yvalue=(l.*(l+1).*psdat(i).sCl./2./pi);
ebar=(l.*(l+1).*psdat(i).dsCl./2./pi);
ebarlow=ebar;
for ii=1:numel(l)
if yvalue(ii)<=ebar(ii)
    ebarlow(ii)=yvalue(ii)-1e-8;
...
legend([pltrCl,pltCl,pltrn,pltph],{'raw','cal',...
    'RN','photon noise'},...
...
%%% before RN sub
yvalue=(l.*(l+1).*psdat(i).sClnw./2./pi);
ebar=(l.*(l+1).*psdat(i).dsClnw./2./pi);
ebarlow=ebar;
for ii=1:numel(l)
if yvalue(ii)<=ebar(ii)
    ebarlow(ii)=yvalue(ii)-1e-8;
